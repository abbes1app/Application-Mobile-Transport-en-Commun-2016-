package com.example.getgpslocation.activity;
...
public class RegisterActivity extends Activity {
    ...
    private EditText inputPassword,inputcPassword;
...
if(password.length()<8){
    Toast.makeText(getApplicationContext(),
            "le Mot de passe doit contenir au moins 8 caractere ", Toast.LENGTH_LONG)
            .show();
}
              else  if ((!name.isEmpty() && !email.isEmpty() && !password.isEmpty() && !cpassword.isEmpty()) && (cpassword.equals(password))) {
                    registerUser(name, email, password);
                } else {
                    Toast.makeText(getApplicationContext(),
                            "Verifié vs champs", Toast.LENGTH_LONG)
                            .show();
                }
            }
        });
